...
labels = np.argmax(out_binders, axis=-1)
min_value, max_value = np.amin(out_binders, axis=0), np.amax(out_binders, axis=0)
opacity = (out_binders - min_value) / (max_value - min_value)
...
handles = [plt.Rectangle((0, 0), 0, 0, color=cm(int(i)), label=name) for i, name in enumerate(names_binders)]
blacks = np.zeros_like(label_image)

plt.imshow(blacks, cmap="gray")
plt.imshow(label_image, cmap=cm, vmin=0, vmax=cm.N, alpha=opacity_image)
...
for i in range(regression.coef_.shape[-1]):
    ...
    plt.plot(peaks, regression.coef_[..., i])
    plt.show()
