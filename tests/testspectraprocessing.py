...
class TestSpectraProcessing(unittest.TestCase):
    ...
    def test_peak_indices(self):
        ...
        indices = sp.peak_indices(y_values, 0.5, 10)
    ...
    def test_find_isotopic_pattern(self):
        L = [[10.2, 10.], [11, 23.], [12.23, 11.25],
             [13.23, 8.75], [14.29, 6.2],[17.21, 20.1], [18.19, 7.5]]
        pattern = sp.find_isotopic_pattern(np.array(L), 0.1, 2.5)
        print(pattern)
        nptest.assert_equal(pattern, [L[0], L[2], L[3], L[4]])

    def test_peaks_isotopic_pattern(self):
        L = np.array([[10.2, 10.], [11, 23.], [12.23, 11.25],
             [13.23, 8.75], [14.3, 6.2], [15.2, 18.96],
             [16.2, 38.2], [17.21, 20.1], [18.19, 7.5]])
        pattern = sp.find_isotopic_pattern(L, 0.1, 5)
        d = sp.forward_derivatives(pattern)
        peaks = sp.peaks_derivative_isotopic_pattern(pattern)
        nptest.assert_equal(peaks[1], L[5])
    ...
    def test_isotopes_from_pattern(self):
        pattern = [[10.2, 10.0], [12.23, 11.25], [13.23, 8.75], [15.2, 18.96]]
        peaks = sp.peaks_derivative_isotopic_pattern(pattern)
        isotopes = sp.isotopes_from_pattern(pattern, peaks)
        nptest.assert_equal(isotopes, pattern[1:3])
