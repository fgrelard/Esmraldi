...
class TestTheoreticalSpectra(unittest.TestCase):
    def setUp(self):
        ...
        k = sr.SpeciesRule("K", "A", mz=39, count=1, naming_fn=lambda i: "K+", family_number=2, adduct_fn=r"AX.*")
        ac = sr.SpeciesRule("Ac", "A", mz=42.02, count=3, naming_fn=lambda i: str(i) + "Ac", family_number=1, adduct_fn=r"AX.*")
    ...
    def test_expand_mix(self):
        mix = self.ts.mix_adducts(self.ts.adducts)
        mix_dict = self.ts.expand_mix(mix[0])
        print(mix_dict)

    def test_mix_adducts(self):
        mix = self.ts.mix_adducts(self.ts.adducts)
        for m in mix:
            for adduct in m:
    ...
    def test_add_all_adducts_to_molecules(self):
        self.ts.add_all_adducts_to_molecules()
        print(self.ts.spectrum)
